...
function removeFirstOccurrences(str, value) {
    return str.substr( 0, str.indexOf(value) ) 
...
function encodeToRot13(str) {
    const   charCodeOfBigZ = 'Z'.charCodeAt(0),
            charCodeOfSmallZ = 'z'.charCodeAt(0),
            charCodeOfBigA = 'A'.charCodeAt(0),
            charCodeOfSmallA = 'a'.charCodeAt(0)
            
    let result = ''

    for (let i = 0; i < str.length; i++) {
        let charCode = str.charCodeAt( i )
        if ( charCode >= charCodeOfBigA && charCode <= charCodeOfSmallZ ){

            if ( charCode > charCodeOfBigZ - 13 && charCode < charCodeOfSmallA ) {
                result += String.fromCharCode( charCodeOfBigA + ( charCode+12 ) - charCodeOfBigZ )
            } else if ( charCode > charCodeOfSmallZ - 13 ) {
                result += String.fromCharCode( charCodeOfSmallA + ( charCode+12 ) - charCodeOfSmallZ )
            } else {
